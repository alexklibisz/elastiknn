...
object LuceneJaccardIndexedQuery extends ElastiKnnVectorUtils {
  ...
  class JaccardIndexedQuery(val field: String, val sim: Similarity, val queryVector: ElastiKnnVector) extends Query {
    ...
    class ExactSimWeight(searcher: IndexSearcher) extends Weight(this) {
      ...
      override def scorer(context: LeafReaderContext): Scorer = {
        ...
        new ExactSimScorer(this, searcher, context, existsIterator)
      }

      override def isCacheable(ctx: LeafReaderContext): Boolean = false
    }

    class ExactSimScorer(weight: Weight, searcher: IndexSearcher, context: LeafReaderContext, iterator: DocIdSetIterator)
      ...
      override def score(): Float = {
        ...
        if (binaryDocValues.advanceExact(docId)) {
          ...
          val ekv2 = {
            val bv2 = searcher.doc(docId).getField(field).binaryValue.bytes
            ElastiKnnVector.parseFrom(bv2.take(bv2.length))
          }
          val numTrue = searcher.doc(docId).getField("num_true").numericValue.longValue
          println((ekv, ekv2, numTrue))
    ...
    override def equals(other: Any): Boolean = other match {
      case esq: JaccardIndexedQuery => esq.field == this.field && esq.sim == this.sim
      case _                        => false
    }
  ...
  def main(args: Array[String]): Unit = {
    ...
    sbvOpts.zipWithIndex.foreach {
      case (sbvOpt, i) =>
        ...
        sbvOpt.foreach { sbv =>
          ...
          doc.add(new StoredField(field, ekv.toByteArray))
          doc.add(new StoredField("num_true", sbv.trueIndices.length))
        }
    ...
    val topDocs = ixSearcher.search(new JaccardIndexedQuery(field, SIMILARITY_JACCARD, queryVector), 10)
