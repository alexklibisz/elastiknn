...
import com.klibisz.elastiknn.{Elastic4sMatchers, ElasticAsyncClient, JaccardLshOptions, ProcessorOptions}
...
class PrepareMappingSuite extends AsyncFunSuite with Matchers with Elastic4sMatchers with ElasticAsyncClient with ElasticDsl {
  ...
  test("prepare mapping works with existing field") {
    ...
    for {
      ...
      _ <- eknn.prepareMapping(index, ProcessorOptions(rawField, 9, ModelOptions.Jaccard(JaccardLshOptions(0, procField))))
