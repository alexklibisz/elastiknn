...
object TestData extends XContentCodecCirceBridge {
  ...
  def genSparseBool(dims: Int, numCorpus: Int, numQueries: Int, numNeighbors: Int)(using rng: Random): TestData = {
  ...
  def genDenseFloat(dims: Int, numCorpus: Int, numQueries: Int, numNeighbors: Int, unit: Boolean = false)(using
      rng: Random
  ): TestData = {
...
trait XContentCodecCirceBridge {
  given encoderFromXContent[T: XContentCodec.Encoder]: Encoder[T] = new Encoder[T] {
  ...
  given fromXContentCodecDecoder[T: XContentCodec.Decoder]: Decoder[T] = new Decoder[T] {
