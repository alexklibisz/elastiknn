...
object KnnQueryBuilderOld {

  val NAME = s"${ELASTIKNN_NAME}_knn_old"

  object Parser extends QueryParser[KnnQueryBuilderOld] {

    /** This is the first method that gets hit when you run this query. */
    override def fromXContent(parser: XContentParser): KnnQueryBuilderOld = {
      ...
      new KnnQueryBuilderOld(query)
    }

  }

  object Reader extends Writeable.Reader[KnnQueryBuilderOld] {

    /** This is uses to transfer the query across nodes in the cluster. */
    override def read(in: StreamInput): KnnQueryBuilderOld = {
      ...
      new KnnQueryBuilderOld(KNearestNeighborsQuery.parseBase64(in.readString()))
    }
  }
...
/**
  ...
  */
final class KnnQueryBuilderOld(val query: KNearestNeighborsQuery, processorOptions: Option[ProcessorOptions] = None)
    extends AbstractQueryBuilder[KnnQueryBuilderOld]
    with CirceUtils {
  ...
  override def doEquals(other: KnnQueryBuilderOld): Boolean = this.query == other.query

  override def doHashCode(): Int = Objects.hash(query)

  override def getWriteableName: String = KnnQueryBuilderOld.NAME
      ...
      processorOptions match {
        ...
        case Some(popts) =>
          (popts.modelOptions, query.queryOptions, ekv, models.processVector(popts, ekv)) match {
            ...
            case (
                ModelOptions.JaccardIndexed(mopts),
                QueryOptions.JaccardIndexed(qopts),
                ElastiKnnVector(ElastiKnnVector.Vector.SparseBoolVector(sbv)),
                Success(proc: ProcessedVector.JaccardIndexed)
                ) =>
              new JaccardIndexedQueryBuilder(mopts, qopts, sbv.sorted(), proc, popts.fieldRaw, query.useCache)
  ...
  private def rewriteFetchQueryVector(context: QueryRewriteContext, iqv: IndexedQueryVector): QueryBuilder = {
    ...
    val supplier = new SetOnce[KnnQueryBuilderOld]()
          ...
          def onResponse(response: GetResponse): Unit =
            extractNested(s"${iqv.field}.".split('.'), response.getSourceAsString)
              .map { json =>
                ...
                supplier.set(new KnnQueryBuilderOld(query.withGiven(ekv), processorOptions))
  ...
  private def rewriteFetchProcessorOptions(context: QueryRewriteContext): QueryBuilder = {
    val maybeCached: ProcessorOptions = KnnQueryBuilderOld.procOptsCache.getIfPresent(query.pipelineId)
    if (maybeCached != null) new KnnQueryBuilderOld(query, Some(maybeCached))
    else {
        ...
        c.execute(
          ...
          new ActionListener[GetPipelineResponse] {
            override def onResponse(response: GetPipelineResponse): Unit = {
              ...
              KnnQueryBuilderOld.procOptsCache.put(query.pipelineId, processorOptions)
              supplier.set(new KnnQueryBuilderOld(query, Some(processorOptions)))
              l.asInstanceOf[ActionListener[Any]].onResponse(null)
            }
...
object ExactComputedQueryBuilder {
  ...
  val NAME = s"${KnnQueryBuilderOld.NAME}_exact_computed"
  ...
  object Parser extends QueryParser[ExactComputedQueryBuilder] {
    override def fromXContent(parser: XContentParser): ExactComputedQueryBuilder =
      throw new IllegalStateException(s"Use the ${KnnQueryBuilderOld.NAME} query instead")
  }
...
object JaccardIndexedQueryBuilder {
  val NAME = s"${KnnQueryBuilderOld.NAME}_indexed_jaccard"

  object Parser extends QueryParser[JaccardIndexedQueryBuilder] {
    override def fromXContent(parser: XContentParser): JaccardIndexedQueryBuilder =
      throw new IllegalStateException(s"Use the ${KnnQueryBuilderOld.NAME} query instead")
  }

  object Reader extends Writeable.Reader[JaccardIndexedQueryBuilder] {
    override def read(in: StreamInput): JaccardIndexedQueryBuilder = {
      in.readFloat()
      in.readOptionalString()
      new JaccardIndexedQueryBuilder(
        modelOptions = JaccardIndexedModelOptions.parseBase64(in.readString()),
        queryOptions = JaccardIndexedQueryOptions.parseBase64(in.readString()),
        queryVector = SparseBoolVector.parseBase64(in.readString()),
        processedQueryVector = io.circe.parser.decode[ProcessedVector.JaccardIndexed](in.readString()).right.get,
        fieldRaw = in.readString(),
        useCache = in.readBoolean()
      )
    }

    def write(b: JaccardIndexedQueryBuilder, out: StreamOutput): Unit = {
      out.writeString(b.modelOptions.toBase64)
      out.writeString(b.queryOptions.toBase64)
      out.writeString(b.queryVector.toBase64)
...
final class JaccardIndexedQueryBuilder(val modelOptions: JaccardIndexedModelOptions,
                                       val queryOptions: JaccardIndexedQueryOptions,
                                       val queryVector: SparseBoolVector,
                                       val processedQueryVector: ProcessedVector.JaccardIndexed,
                                       val fieldRaw: String,
                                       val useCache: Boolean)
    extends AbstractQueryBuilder[JaccardIndexedQueryBuilder] {

  override def doWriteTo(out: StreamOutput): Unit = JaccardIndexedQueryBuilder.Reader.write(this, out)

  override def doXContent(builder: XContentBuilder, params: ToXContent.Params): Unit = ()

  override def doToQuery(context: QueryShardContext): Query = {
    val mq: MatchQueryBuilder =
      new MatchQueryBuilder(s"${modelOptions.fieldProcessed}.JaccardIndexed.trueIndices", processedQueryVector.trueIndices)
    val ft: MappedFieldType = context.getMapperService.fullName(s"${modelOptions.fieldProcessed}.JaccardIndexed.numTrueIndices")
  ...
  override def doEquals(other: JaccardIndexedQueryBuilder): Boolean =
    this.modelOptions == other.modelOptions &&
      this.queryOptions == other.queryOptions &&
      this.queryVector == other.queryVector &&
  ...
  override def getWriteableName: String = JaccardIndexedQueryBuilder.NAME
}

object JaccardLshQueryBuilder {
  val NAME = s"${KnnQueryBuilderOld.NAME}_jaccard_lsh"
