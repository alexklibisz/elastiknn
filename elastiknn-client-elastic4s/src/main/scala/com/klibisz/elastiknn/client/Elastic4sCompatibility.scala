...
import com.klibisz.elastiknn.api.{NearestNeighborsQuery, XContentEncoder}
...
trait Elastic4sCompatibility {
  ...
  implicit class NearestNeighborsQueryCompat(nnq: NearestNeighborsQuery) {
    def toQuery: Query = RawQuery(s"""{"elastiknn_nearest_neighbors":${XContentEncoder.encodeUnsafeToString(nnq)}}""")
  }
}
