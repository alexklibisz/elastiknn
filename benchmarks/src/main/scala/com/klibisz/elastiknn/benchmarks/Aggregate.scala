...
object Aggregate extends App {
  ...
  final case class CsvRow(dataset: String, algorithm: String, k: Int, mappingJson: String, averageRecall: Double, queriesPerSecondPerShard: Double)
  ...
  def apply(params: Params): ZIO[Any, Throwable, Unit] = {
    ...
    val layer =
      (Blocking.live ++ ZLayer.succeed(s3Client)) >>>
          ResultClient.s3(params.resultsBucket, params.resultsPrefix) ++ Blocking.live

    val logic: ZIO[Blocking with ResultClient, Throwable, Unit] = for {
      ...
      rows = results.map { res =>
        CsvRow(
          ...
          res.queryResults.map(_.recall).sum / res.queryResults.length,
      ...
      writer = csvFile.asCsvWriter[CsvRow](rfc.withHeader)
      _ = rows.foreach(row => ZIO(writer.write(row)))
      _ = writer.close()
